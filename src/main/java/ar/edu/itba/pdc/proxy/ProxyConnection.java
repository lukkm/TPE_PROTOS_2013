...
public class ProxyConnection {
	...
	public int readFrom(SocketChannel s) throws IOException {
		/*
		 * Ver que hago con reads mas grandes que el buffer. Inicialmente es
		 * transparente, ya que va a volver a entrar con otro read Mas adelante
		 * hay que ver como procesar esos paquetes.
		 */
		...
		if (bytesRead != -1)
			message = new String(buffersMap.get(s).getBufferArray(
					BufferType.read)).substring(0, bytesRead);
	...
	public void send(SocketChannel s, Stanza stanza) {
		System.out.println("SEND NOMAS:"
				+ new String(stanza.getXMLString().getBytes()));
